---
title: "Multimodal network passengers copie"
output:
  html_document:
    df_print: paged
---
...
coordinates <- read.csv(file='/Users/francois-xavier/Desktop/Statbel/Thesis/Thesis/wld_trs_ports_wfp.csv', header=T)

# Remove all columns from the coordinate data except for columns 3, 12, and 13
coordinates <- coordinates[,c(3,12,13)]

# Rename the "code" column to "label"
coordinates<-rename(coordinates,label=code )

# Sort the coordinate data by the "label" column
coordinates<- coordinates[order(coordinates$label),]

# Filter out any rows where the "label" column is blank or contains only whitespace
coordinates<- filter(coordinates,coordinates$label!="")
coordinates<- filter(coordinates,coordinates$label!=" ")

# Create a new data frame called "n_occur" that shows the frequency of occurrence for each value in the "label" column of the coordinate data
n_occur <- data.frame(table(coordinates$label))

# Remove rows from the coordinate data with label values of 692, 734, or 832
coordinates<- coordinates[-c(692,734,832),]

# Read in additional coordinate data from the CSV file and store it in a variable called "coo"
coo <-read.csv(file = "/Users/francois-xavier/Desktop/Statbel/Thesis/Thesis/testtttttt.csv",header=T)

# Keep only columns 3, 12, and 13 from the "coo" data frame
coo<- coo[,c(3,12,13)]

# Rename the columns of the "coo" data frame to "label", "latitude", and "longitude"
colnames(coo)<-c("label","latitude","longitude")

# Combine the "coo" data frame with the existing "coordinates" data frame and sort by the "label" column
coordinates<-rbind(coo,coordinates)
coordinates<-coordinates[order(coordinates$label),]

# Remove any duplicated rows from the "coordinates" data frame, keeping only the first occurrence of each label value
coordinates<-coordinates[!duplicated(coordinates$label),]

# Remove the "n_occur" data frame from memory
rm(n_occur)

full_data <- read.csv(file="/Users/francois-xavier/Desktop/Statbel/Thesis/Thesis/DFCaseStudyFinal.csv")
...
# Read in the goods data from the CSV file and store it in a variable called "goods"
goods <- read.csv(file='C:/Users/FX.DeDecker/OneDrive - GCloud Belgium/Bureau/thesis/Data/goods2018to2022.csv', header=T)

# Remove the first column from the "goods" data frame
goods <-goods[,-1]

# Add a new column called "transport_type" to the "goods" data frame with a value of "MAR" for all rows
goods$transport_type <- "MAR"


# Take the first character of the "cd_lod_type" column in the "goods" data frame and store it as the new value for that column
goods$cd_lod_type <- substr(goods$cd_lod_type,1,1)

# Replace any "0" values in the "cd_lod_type" column of the "goods" data frame with "9"
goods$cd_lod_type[goods$cd_lod_type=="0"] <-"9"

# Read in the inland data from the CSV file and store it in a variable called "inland"
inland<- read.csv(file='C:/Users/FX.DeDecker/OneDrive - GCloud Belgium/Bureau/thesis/Data/Inland.csv', header=T)

# Remove the first column from the "inland" data frame
inland<-inland[,-1]

# Add a new column called "transport_type" to the "inland" data frame with a value of "Inl" for all rows
inland$transport_type <- "Inl"

# Read in a shapefile called "Highway" and store it in a variable of the same name
Highway <- read_sf("C:/Users/FX.DeDecker/OneDrive - GCloud Belgium/Bureau/thesis/R/Highway.shp")

# Combine the "goods" and "inland" data frames into a new data frame called "dataF"
dataF <- rbind(goods,inland)

# Change any "BEGEN" values in the "Destination" column of the "dataF" data frame to "BEGNE"
dataF[dataF$Destination=="BEGEN",]$Destination<-"BEGNE"

# Change any "BEGEN" values in the "Source" column of the "dataF" data frame to "BEGNE"
dataF[dataF$Source=="BEGEN",]$Source<-"BEGNE"

# Write the "dataF" data frame to a CSV file called "datafull.csv"
write.csv(dataF,file="C:/Users/FX.DeDecker/OneDrive - GCloud Belgium/Bureau/thesis/Data/datafull.csv")

# Read in the statistics data from the CSV file and store it in a variable called "full_data"
full_data <- read.csv(file="C:/Users/FX.DeDecker/OneDrive - GCloud Belgium/Bureau/thesis/Data/statistics2018to2022.csv")[,-1]

# Remove the "goods", "inland", and "passengers" data frames from memory
rm(goods)
rm(inland)
rm(passengers)
```
...
create function to prepare 5 dataframes : Departures, Arrivals and both, and aggregated by contry for dep and arivals
```{r echo=FALSE}
...
Evolution_importance_Location <- function(Location){
  ...
  # Set the plotting layout to a 2x2 grid
  par(mfrow = c(2, 2))
  
  # Loop over columns 12 through 15 in 'table_Location'
  for(i in 12:15){
    
    # Plot the i-th column against the 'date' column in 'table_Location'
    # The 'type = "o"' argument specifies that the plot should be a line plot with points
    # The 'xlab' and 'ylab' arguments specify the x and y axis labels respectively
    # The 'main' argument specifies the title of the plot
    plot(as.Date(table_Location$date),table_Location[,i],type = "o",xlab = "", ylab = "",main=names(table_Location)[i])
    
    # Add a red line to the plot using a linear regression model of the i-th column against the 'date' column in 'table_Location'
    abline(lm(table_Location[,i]~as.Date(table_Location$date)),col="red")
  }
}

Evolution_importance_Location("BELEU")
```
...
gfg<-st_read("/Users/francois-xavier/Desktop/Statbel/Thesis/Thesis/WaternetworkFinal.shp")
# Read shapefile and convert to sf
#gfg <- graphFull %>% activate(edges) %>% as_tibble() %>% st_as_sf()
...
ports <- read.csv(file='/Users/francois-xavier/Desktop/Statbel/Thesis/Thesis/coordinatesaggregatedOutsideEUNOTagreggatedEU.csv')[,-1]
...
dataF <- read.csv(file = "/Users/francois-xavier/Desktop/Statbel/Thesis/Thesis/datafull.csv")[,-1]
      ...
      fluidRow(tags$style(
        ...
        column(width = 12,
               tabsetPanel(
                ...
                tabPanel("Statistics Time",
                          plotOutput("Time", height = "500px", width = "700px"),
                          style = "background-color: #eeeeee;"
                          )
